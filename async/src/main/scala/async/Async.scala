...
object Async {
  ...
  /**
    ...
    */
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = eventuallyX.map( (x: Int) => (x % 2) == 0 )
  ...
  def insist[A](makeAsyncComputation: () => Future[A], maxAttempts: Int): Future[A] = {
    val comp = makeAsyncComputation()
    comp.recoverWith(
      {case t => {
        if (maxAttempts > 1) insist(makeAsyncComputation, maxAttempts -1) else Future.failed(t)
      }
    })
  }
}
